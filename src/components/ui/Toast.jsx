...
export function ToastProvider({ children }) {
  ...
  const iconMap = {
    success: <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"><path d="M22 11.08V12a10 10 0 1 1-5.93-9.14"/><polyline points="22 4 12 14.01 9 11.01"/></svg>,
    error: <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"><circle cx="12" cy="12" r="10"/><line x1="15" y1="9" x2="9" y2="15"/><line x1="9" y1="9" x2="15" y2="15"/></svg>,
      ...
      <div className="kfpl-toast-container">
        {toasts.map(toast => (
          <div key={toast.id} className={`kfpl-toast ${toast.type}`}>
            <div className="kfpl-toast-icon">{iconMap[toast.type]}</div>
            <div className="kfpl-toast-content">
              {toast.title && <div className="kfpl-toast-title">{toast.title}</div>}
              <div className="kfpl-toast-message">{toast.message}</div>
            </div>
            <button className="kfpl-toast-close" onClick={() => removeToast(toast.id)}>
